...
Optimized for RunPod RTX 4090: Sequential GPU self-play + Ultralight game logic.

This version avoids the overhead of multiprocessing and CPU inference,
running everything on the GPU for maximum reliability and speed.
"""
import torch
...
class GoDataset(Dataset):
    def __init__(self, states, policies, values):
        # Efficiently convert to tensors
...
# ---------------------------------------------------------------------------
# GPU Self-Play (Sequential & Fast)
# ---------------------------------------------------------------------------
def gpu_selfplay(model, device, num_games, mcts_sims, in_channels):
    """Run self-play games sequentially on GPU. 
    With optimized Go logic, this is faster and simpler than multiprocessing."""
    ...
    for g in range(num_games):
        ...
        print(f"  [game {g+1}/{num_games}] {len(states)} moves, {game_time:.1f}s, result: {res:.1f}", flush=True)
...
def train(args):
    ...
    print(f"[train] Device: {device} | Mode: Sequential GPU Self-play")
    ...
    for cycle in range(start_cycle, args.cycles):
        ...
        # 1. Self-play
        print(f"[selfplay] Playing {args.games_per_cycle} games on GPU...")
        s, p, v, res = gpu_selfplay(model, device, args.games_per_cycle, args.mcts_sims, IN_CHANNELS)
...
def main():
    ...
    if args.dry_run: args.games_per_cycle, args.train_epochs = 2, 1
    train(args)
